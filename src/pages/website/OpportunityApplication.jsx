...
const OpportunityApplication = () => {
  ...
  const handleCvChange = (event) => {
    ...
    reader.onload = () => setCv({ name: file.name, data: reader.result });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("/api/public/job-applications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, opportunity: job.title, opportunityId: job.id, cvName: cv.name, cvData: cv.data }) });
    ...
    <section className="min-h-screen bg-gray-50 py-16 md:py-24">
      <div className="mx-auto max-w-3xl px-6">
        ...
        <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl">
          <div className="bg-gradient-to-br from-[#72bf24] to-[#5c9f1b] px-8 py-10 text-white"><p className="text-xs font-semibold uppercase tracking-widest text-white/80">Application</p><h1 className="mt-2 text-3xl font-semibold">Apply for {job.title}</h1>{job.location && <p className="mt-3 text-white/85"><i className="bi bi-geo-alt mr-2"></i>{job.location}</p>}</div>
